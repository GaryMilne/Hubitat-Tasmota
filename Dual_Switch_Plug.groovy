/**
*  Tasmota Sync Dual Switch\Plug Driver
*  Version: v0.98.0
...
metadata {
		definition (name: "Tasmota Sync - Dual Switch\\Plug", namespace: "garyjmilne", author: "Gary J. Milne", importUrl: "https://raw.githubusercontent.com/GaryMilne/Hubitat-Tasmota/main/Dual_Switch_Plug.groovy", singleThreaded: true )  {
        capability "Switch"
        capability "Refresh"
    ...
    section("Configure the Inputs"){
            ...
            input name: "switchBehaviour", type: "enum", title: bold("Switch Behaviour."), description: italic("Whether the primary Switch affects only outlet 1 or both outlet 1 and outlet 2.)"),
              options: [ [1:"Turns ON\\OFF only Power1"],[2:"Turns ON\\OFF Power1 & Power2"] ], defaultValue: 1
...
//Turns the Power on to one or both switches based on settings. Note: Power and Power1 are synonymous in Tasmota
def on() {
    if (settings.switchBehaviour == "1") {
        ...
        {
        log("Action", "Turn on both switches", 0)
        command = "POWER1 ON; POWER2 ON"
        callTasmota("BACKLOG", command)
        }
    }
        
//Turns off one or both switches.
def off() {
    if (settings.switchBehaviour == "1") {
        ...
        {
        log("Action", "Turn off both switches", 0)
        command = "POWER1 OFF; POWER2 OFF"
        callTasmota("BACKLOG", command)
        }
	}
...
def tasmotaInjectRule(){
    ...
    rule3 = rule3 + "ON Power2#state DO backlog0 Var14 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Rules#Timer=1 DO Var15 %Var13%,%Var14% ENDON "
    rule3 = rule3 + "ON Var15#State\$!%Var16% DO backlog ; Var16 %Var15% ; webquery http://" + settings.HubIP + ":39501 POST {'TSync':'True','Switch1':'%Var13%','Switch2':'%Var14%'} ENDON "
