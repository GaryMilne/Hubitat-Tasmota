/**
*  Tasmota Sync 8 Port Relay\Switch Driver
*  Version: v0.98.1
*  Download: See importUrl in definition
*  Description: Hubitat Driver for Tasmota 8 Port Relay\Switch. Provides Realtime and native synchronization between Hubitat and Tasmota
*
...
metadata {
		definition (name: "Tasmota Sync - 8 Port Relay\\Switch", namespace: "garyjmilne", author: "Gary J. Milne", importUrl: "https://raw.githubusercontent.com/GaryMilne/Hubitat-Tasmota/main/8_Port_Relay_Switch.groovy", singleThreaded: true )  {
        capability "Switch"
        capability "Refresh"
    ...
    section("Configure the Inputs"){
            ...
            input name: "switchBehaviour", type: "enum", title: bold("Switch Behaviour."), description: italic("Whether the primary Switch affects only Relay 1 or ALL Relays 1 - 8."),
              options: [ [1:"Turns ON\\OFF only Power1"],[2:"Turns ON\\OFF ALL Relays"] ], defaultValue: 1
...
def on() {
    if (settings.switchBehaviour == "1") {
        log("Action", "Turn on switch1", 0)
        callTasmota("POWER1", "ON")
        }
    else 
        {
        log("Action", "Turn on all switches", 0)
        command = "POWER1 ON; POWER2 ON; POWER3 ON; POWER4 ON; POWER5 ON; POWER6 ON; POWER7 ON; POWER8 ON"
...
def off() {
    if (settings.switchBehaviour == "1") {
        log("Action", "Turn off switch1", 0)
        callTasmota("POWER1", "OFF")
        }
    else 
        {
        log("Action", "Turn off all switches", 0)
        command = "POWER1 OFF; POWER2 OFF; POWER3 OFF; POWER4 OFF; POWER5 OFF; POWER6 OFF; POWER7 OFF; POWER8 OFF"
        callTasmota("BACKLOG", command)
        }
	}
    ...
    if (body !=null){
        ...
        switch(Action.toUpperCase()) { 
            ...
            case ["STATE"]:
                ...
                sendEvent(name: "switch2", value: body.POWER2.toLowerCase(), displayed:false)
                sendEvent(name: "switch3", value: body.POWER3.toLowerCase(), displayed:false)
                sendEvent(name: "switch4", value: body.POWER4.toLowerCase(), displayed:false)
                sendEvent(name: "switch5", value: body.POWER5.toLowerCase(), displayed:false)
                sendEvent(name: "switch6", value: body.POWER6.toLowerCase(), displayed:false)
                sendEvent(name: "switch7", value: body.POWER7.toLowerCase(), displayed:false)
                sendEvent(name: "switch8", value: body.POWER8.toLowerCase(), displayed:false)
                updateStatus("Complete:Success")
            	break            
...
def syncTasmota(body){
    ...
    if (elapsed > settings.debounce){
        ...
        if (body?.SWITCH2 != '') { switch2 = body?.SWITCH2 ; log ("syncTasmota","Switch is: ${switch2}", 2) }
		if (body?.SWITCH3 != '') { switch3 = body?.SWITCH3 ; log ("syncTasmota","Switch is: ${switch3}", 2) }
		if (body?.SWITCH4 != '') { switch4 = body?.SWITCH4 ; log ("syncTasmota","Switch is: ${switch4}", 2) }
		if (body?.SWITCH5 != '') { switch5 = body?.SWITCH5 ; log ("syncTasmota","Switch is: ${switch5}", 2) }
		if (body?.SWITCH6 != '') { switch6 = body?.SWITCH6 ; log ("syncTasmota","Switch is: ${switch6}", 2) }
		if (body?.SWITCH7 != '') { switch7 = body?.SWITCH7 ; log ("syncTasmota","Switch is: ${switch7}", 2) }
		if (body?.SWITCH8 != '') { switch8 = body?.SWITCH8 ; log ("syncTasmota","Switch is: ${switch8}", 2) }
        
        //Now apply any changes that have been found. In Tasmota, "power" is the switch state unless referring to sensor data.
        if ( switch1.toInteger() == 0 ) sendEvent(name: "switch", value: "off", descriptionText: "Switch 1 was turned off.")
        if ( switch1.toInteger() == 1 ) sendEvent(name: "switch", value: "on", descriptionText: "Switch 1 was turned on.")
...
def tasmotaInjectRule(){
    ...
    rule3 = "ON Power1#State DO backlog0 Var6 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Power2#state DO backlog0 Var7 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power3#state DO backlog0 Var8 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power4#state DO backlog0 Var9 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power5#state DO backlog0 Var10 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power6#state DO backlog0 Var11 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power7#state DO backlog0 Var12 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power8#state DO backlog0 Var13 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Rules#Timer=1 DO Var15 %Var6%,%Var7%,%Var8%,%Var9%,%Var10%,%Var11%,%Var12%,%Var13% ENDON "
    rule3 = rule3 + "ON Var15#State\$!%Var16% DO backlog ; Var16 %Var15% ; webquery http://" + settings.HubIP + ":39501 POST {'TSync':'True','Switch1':'%Var6%','Switch2':'%Var7%','Switch3':'%Var8%','Switch4':'%Var9%','Switch5':'%Var10%','Switch6':'%Var11%','Switch7':'%Var12%','Switch8':'%Var13%'} ENDON "
