...
*
*  PLUG - UNIQUE - CHANGELOG
*  Version 0.91 - Internal version
*  Version 0.92 - Allow device handler to operate for Plug or Plug with Power Monitoring
...
metadata {
        ...
        command "tasmotaTelePeriod", [ [name:"Seconds*", type: "STRING", description: "The number of seconds between Tasmota data updates (TelePeriod XX). Only applies to Switches with Power Monitoring."] ]
        command "toggle"
        //command "test"
        ...
        //attribute "frequency", "number"      //unit Hz (part of VoltageMeasurement)  
...
def initialize(){
    ...
    //Simple switch - no PM
    if (settings.plugType.toInteger() == 0 ) {
        state.plugType = "Simple switch"
    ...
    if (settings.plugType.toInteger() == 1 ) {
        state.plugType = "Lite Power Monitoring (Watts only)."
    ...
    //Plug with PM. Only log switch and power events
    if (settings.plugType.toInteger() == 2 ) {
        state.plugType = "Full Power Monitoring (Current, Voltage and Watts)."
        log("Initialize", "Configured as switch with power monitoring. Logging ALL switch, power, voltage and amps events.", 0)
        sendEvent(name: "current", value: 0, descriptionText: "Current events have been enabled", unit: "Amps", isStateChange: true )
        sendEvent(name: "voltage", value: 0, descriptionText: "Voltage events have been enabled", unit: "Volts", isStateChange: true )
        sendEvent(name: "power", value: 0, descriptionText: "Power (watts) events have been enabled", unit: "Watts", isStateChange: true )
    }
...
def hubitatResponse(body){
    ...
    if (body !=null && tasmotaWarning == false ){
...
def syncTasmota(body){
    ...
    if (elapsed > settings.debounce){
        ...
        switchVal = -1 ; current = -1 ; voltage = -1 ; watts = -1
        ...
        if (body?.WATTS != '') { watts = body?.WATTS ; log ("syncTasmota","Watts is: ${watts}", 2) }
        ...
        //Only changes will get logged so we can report everything. In Tasmota, "power" is the switch state.
        ...
        if (settings.plugType.toInteger() >= 1 ){ 
            if ( watts.toInteger() >= 0 ) sendEvent(name: "power", value: watts, unit: "Watts") 
            }
...
def tasmotaInjectRule(){
    ...
    //Simple Plug - no power monitoring
    if ( settings.plugType.toInteger() == 0 ) {
        rule3 = "ON Power1#state DO webquery http://" + settings.HubIP + ":39501/ POST {'TSync':'True','Switch':'%value%','Current':'','Voltage':'','Watts':''} ENDON "
        }
    
    //Plug monitoring only Watts
    if ( settings.plugType.toInteger() == 1 ) {
        rule3 = "ON Power1#state DO backlog var11 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Tele-Energy#Power DO backlog0 var14 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Rules#Timer=1 DO var15 %var11%,%var14% ENDON "
        rule3 = rule3 + "ON var15#State\$!%var16% DO backlog ; var16 %var15% ; webquery http://" + settings.HubIP + ":39501/ POST {'TSync':'True','Switch':'%var11%','Current':'','Voltage':'','Watts':'%var14%'} ENDON "
    }
    
    //Plug monitoring everything
    if ( settings.plugType.toInteger() == 2 ) {
        rule3 = "ON Power1#state DO backlog var11 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Tele-Energy#Current DO backlog0 var12 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Tele-Energy#Voltage DO backlog0 var13 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Tele-Energy#Power DO backlog0 var14 %value% ; RuleTimer1 1 ENDON "
        rule3 = rule3 + "ON Rules#Timer=1 DO var15 %var11%,%var12%,%var13%,%var14% ENDON "
        rule3 = rule3 + "ON var15#State\$!%var16% DO backlog ; var16 %var15% ; webquery http://" + settings.HubIP + ":39501/ POST {'TSync':'True','Switch':'%var11%','Current':'%var12%','Voltage':'%var13%','Watts':'%var14%'} ENDON "
    }
    
    //Now install the rule onto Tasmota
    callTasmota("RULE3", rule3)
    
    //Now make sure the rule is turned on.
    command = "RULE3 ON"
