...
*
*  This driver is one of several in the Tasmota Sync series. (Bulb, Plug, Dual Outlet, Dimmer Switch, Switch with Fan, Switch with Sensor (Temp & Humidity). All of these drivers are very similar and much of the code is identical.
...
*
*  BULB - UNIQUE - CHANGELOG
*  Version 0.91 - Internal version
*  Version 0.92 - Added logic for extracting HSV values from color and updating the attributes on any change. Added hue, saturation and value attributes for Hubitat compatibility.
...
*  Version 0.98 - Updated cleanURL to support "+". Updated all color functions to use COLOR when dimmer at 100% and COLOR2 when dimmer less than 100% to preserve dimmer value.
*
* Authors Notes:
* You might be asking yourself, do we really need another Tasmota driver for Hubitat? It's a fair question as there are already multiple drivers available. By far the best is the "Tasmota Device Manager" by Markus Li.
* That said, the version by Markus is no longer supported and Markus has been banned from the Hubitat platform. I also found several other things about Markus' version to be limiting.
*  1) To use Markus' version you had to use a special compilation of Tasmota that introduced some Hubitat hooks for synchronisation. 
...
*  2) Markus used a Parent\Child App\Driver architecture. Meaning that a simple Tasmota device like a Bulb or a Switch had two driver entries. I found this unweildy and cumbersome.
*     Tasmota Sync drivers are just that, drivers only. Each Tasmota device (IP address) will only have one entry in the driver table making it a little cleaner.
*  3) Markus' implementation was quite chatty and would sync on changes and at each TelePeriod (usually every 5 minutes). 
*     Tasmota Sync drivers only report on changes at the Tasmota device with the exception of the Power Management device which reports at the TelePeriod but ONLY if there has been a change.
*  4) I also found that Markus' drivers were not always complete such as setting colorTemperature on a bulb. Markus' code was also very dense making changes difficult when combined with the App\Parent\Child architecture. 
*     All features of Tasmota Sync drivers are fully operational and the code is more straightfoward and approachable to non veteran coders without comprimising reliability of the driver.
*
*  Lastly, I had previously written much of this code for the Smartthings Classic environment and carried forward some useful elements such as saving and recalling favorite color and CT values and full support for Tasmota Fade.
*  As Hubitat does not have the same UI that Smartthings Classic does, I've done some innovative things with logging that make it much easier to troubleshoot problems should some arise, and more fun to look at when they don't.
*
*  Gary Milne - May, 2022
*
**/
...
metadata {
        ...
        command "colorApply", [ [name:"Index*", type: "STRING", description: "Number of saved color to apply."] ]
        command "colorSave", [ [name:"Index*", type: "STRING", description: "Number by which the color can be accessed."], [name:"Description*", type: "STRING", description: "An optional description of the color being saved."] ]
        command "initialize"
        command "toggle"
        //command "test"
        
        attribute "colorTemperatureRange", "string"    //This is required for SharpTools to recognize the full range of CT bulbs
        ...
        attribute "Mireds", "string"
		attribute "Fade", "string"
        attribute "FadeSpeed", "string"  
        attribute "Status", "string"
        
        //Used for Preset colors that can be user modified. The value is set to null when deleted and they will disappear from current states upon a browser refresh.
        attribute "🎨0", "string"
        attribute "🎨1", "string"
        attribute "🎨2", "string"
        attribute "🎨3", "string"
...
def initialize(){
    ...
    if ( device.currentValue("🎨0") == null ) sendEvent(name: "🎨0", value: "00000000FF:Warm White")
    if ( device.currentValue("🎨1") == null ) sendEvent(name: "🎨1", value: "0000007B84:Soft White")
    if ( device.currentValue("🎨2") == null ) sendEvent(name: "🎨2", value: "000000C13E:Cool White")
    if ( device.currentValue("🎨3") == null ) sendEvent(name: "🎨3", value: "000000FF00:Daylight")
    if ( device.currentValue("🎨4") == null ) sendEvent(name: "🎨4", value: "FFC1240000:Early Sunset")
    if ( device.currentValue("🎨5") == null ) sendEvent(name: "🎨5", value: "F2A93A0000:Late Sunset")
    if ( device.currentValue("🎨6") == null ) sendEvent(name: "🎨6", value: "96FF9B0000:Mint Green")
    if ( device.currentValue("🎨7") == null ) sendEvent(name: "🎨7", value: "8ACDF20000:Baby Blue")
    if ( device.currentValue("🎨8") == null ) sendEvent(name: "🎨8", value: "FFBFC70000:Pink")
    if ( device.currentValue("🎨9") == null ) sendEvent(name: "🎨9", value: "00FFD10000:Teal") 
    if ( device.currentValue("colorTemperatureRange") == null ) sendEvent(name: "colorTemperatureRange", value: "1800...6500") 
...
//*********************************************************************************************************************************************************************
//******
//****** UNIQUE: Start of Command functions. These may be UNIQUE across all Tasmota Sync drivers
//******
//*********************************************************************************************************************************************************************
...
void colorApply(String indx) {
    log ("Action - colorApply", "Applying preset color with index ${indx}", 0)
    try {
        index = indx.toInteger()
        if ( index >= 0 && index <= 9) {
            preset = device.currentValue("🎨${index}")
            String[] str
            str = preset.split(':')
            color = str[0]
            description = str[1]
            log ("colorApply", "Applying Preset 🎨${index} color: ${color} Description: (${description})", 1)
            
            //If a dimmer level is set we will preserve it when changing the color.
            if ( device.currentValue('level') == 100 ) callTasmota("COLOR", color )
            else callTasmota("COLOR2", color )
            }
        else { log ("colorApply", "Index (${index}) not in range 0 - 9", -1) }
        }
    catch (Exception e) { log ("colorApply", "Error: Should be a numeric value between 0 and 9.", -1) }
}

void colorSave(String indx, String description) {
    log ("Action - colorSave", "Saving color to index ${indx}", 0)
    try {
        index = indx.toInteger()
        if ( index >= 0 && index <= 9) {
            color = device.currentValue("color")
            sendEvent(name: "🎨${index}", value: "${color}:${description}")
            log ("colorSave", "Saved color: ${color} Description: (${description})", 1)
            }
            else { log ("colorSave", "Index (${index}) not in range 0 - 9", -1) }
        }
    catch (Exception e) { log ("colorSave", "Error: Invalid Index. Should be a numeric value between 0 and 9.", -1) }
}
...
def hubitatResponse(body){
    ...
    if (body !=null){
        ...
        log ("hubitatResponse", "Tasmota settings: Power: ${body.POWER} Dimmer: ${body.DIMMER} Mireds: ${body.CT} Color: ${body.COLOR} Fade: ${body.FADE} FadeSpeed: ${body.SPEED} RSSI: ${state.RSSI}", 2)
        ...
        switch(Action.toUpperCase()) { 
            ...
            case ["COLOR"]:	
                def newColor = ActionValue.toUpperCase()
                def map = isColor(newColor)
                def desiredColor = map.Color
                log("hubitatResponse","Command: COLOR: ${body.COLOR}  Desired COLOR is: ${desiredColor}", 1)
                if ( desiredColor == body.COLOR){
                    log ("hubitatResponse","Color applied successfully", 0)
                	...
                	{
                    log("hubitatResponse","Color state failed to apply", -1)
                    updateStatus("Complete:Fail")
                	}
                break
            
            case ["COLOR2"]:	
                //When we use COLOR2 the color returned does not match the color requested because it is adjusted by the dimmer setting.
                log("hubitatResponse","Command: COLOR2: ${body.COLOR}. Adjusted for dimmer!)", 1)
                if ( body?.COLOR != null){
                    log ("hubitatResponse","Color applied successfully. (Adjusted for dimmer!)", 0)
                    sendEvent(name: "switch", value: "on", displayed:true)
            ...
            case ["DIMMER"]:  //Usually referred to as level in Hubitat
             ...
             case ["CT"]:
                ...
                if (ActionValue.toInteger() == body.CT.toInteger()){
                	log ("hubitatResponse","Color temp applied successfully", 0)
                    sendEvent(name: "switch", value: "on", displayed:true)
                    sendEvent(name: "color", value: desiredColor, displayed:true)
                    sendEvent(name: "level", value: body.DIMMER, displayed:true)
                    sendEvent(name: "Mireds", value: body.CT, displayed:false)
                    ...
                    def currentColor = body?.COLOR
                    //Update the Hubitat color value to match the bulb.
                    log("hubitatResponse","CT: Update color to: ${currentColor}", 1) 
                    sendEvent(name: "color", value: currentColor, displayed:true)
                    setHSVfromColor(body.COLOR)
                    }
            else {
                log("hubitatResponse","Color Temp state failed to apply", -1)
                updateStatus("Complete:Fail")
                }
            	break
            ...
            case ["BACKLOG"]:
            	...
            	//If we come back to this spot we know a BACKLOG command was issued and SOMETHING (usually [:] came back so we know the command at least got to the device.
            ...
            case ["STATE"]:
                ...
                sendEvent(name: "color", value: body.COLOR, displayed:false)
                sendEvent(name: "level", value: body.DIMMER, displayed:false)
                sendEvent(name: "Fade", value: body.FADE.toLowerCase(), displayed:false)
                sendEvent(name: "FadeSpeed", value: body.SPEED, displayed:false)
                sendEvent(name: "Mireds", value: body.CT, displayed:false)
                sendEvent(name: "colorTemperature", value: miredsToKelvin(body.CT), displayed:false)
                setHSVfromColor(body.COLOR)
                updateStatus("Complete:Success")
            	break
...
def syncTasmota(body){
    ...
    if (elapsed > settings.debounce){
        ...
        switchVal = -1 ; color = "" ; dimmer = -1 ; mireds = -1 ; fade = "SAME" ; fadespeed = -1
        ...
        if (body?.SWITCH != '') { switchVal = body?.SWITCH ; log ("syncTasmota","Switch is: ${switchVal}", 2) }
        if (body?.COLOR != '') { color = body?.COLOR ; log ("syncTasmota","Color is: ${color}", 2) }
        if (body?.DIMMER != '') { dimmer = body?.DIMMER.toInteger() ; log ("syncTasmota","Dimmer is: ${dimmer}", 2) }
        if (body?.CT != '') { mireds = body?.CT.toInteger() ; kelvin = miredsToKelvin(mireds); log ("syncTasmota","Mireds: ${mireds}  Kelvin: ${kelvin}", 2) }
        ...
        if ( switchVal.toInteger() == 0 ) sendEvent(name: "switch", value: "off", descriptionText: "The switch was turned off.")
        if ( switchVal.toInteger() == 1 ) sendEvent(name: "switch", value: "on", descriptionText: "The switch was turned on.")
        if ( color != "" ) { sendEvent(name: "color", value: color) ; setHSVfromColor(color) }
        if ( dimmer >= 0 ) sendEvent(name: "level", value: dimmer)
        if ( (mireds.toInteger() ) >= 0 ) { sendEvent(name: "Mireds", value: mireds) ; sendEvent(name: "colorTemperature", value: kelvin) }
...
//*************************************************************************************************************************************************************************************************************
//******
//****** UNIQUE: The only things that gets routed here are responses to requests for Sensor updates. Not used in most drivers.
//******
//*************************************************************************************************************************************************************************************************************
...
def tasmotaInjectRule(){
    ...
    rule3 = "ON Power1#State DO backlog0 Var9 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Color DO backlog0 Var10 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Dimmer DO backlog0 Var11 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON CT DO backlog0 ; Var12 %value% ; RuleTimer1 1 ENDON "
    ...
    rule3 = rule3 + "ON Rules#Timer=1 DO Var15 %Var9%,%color%,%Var11%,%Var12%,%Var13%,%Var14% ENDON "
    ...
    rule3 = rule3 + "ON Var15#State\$!%Var16% DO backlog ; Var16 %Var15% ; webquery http://" + settings.HubIP + ":39501 POST {'TSync':'True','Switch':'%Var9%','Color':'%color%','Dimmer':'%Var11%','CT':'%Var12%','Fade':'%Var13%','Speed':'%Var14%'} ENDON "
