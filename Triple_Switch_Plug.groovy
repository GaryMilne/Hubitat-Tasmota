/**
*  Tasmota Sync Triple Switch\Plug Driver
*  Version: v0.98.1
*  Download: See importUrl in definition
*  Description: Hubitat Driver for Tasmota Triple Switch Plug. Provides Realtime and native synchronization between Hubitat and Tasmota
*
...
*  Version 0.98.1 - Created version for third switch\plug
*
* Authors Notes:
...
metadata {
		definition (name: "Tasmota Sync - Triple Switch\\Plug", namespace: "garyjmilne", author: "Gary J. Milne", importUrl: "https://raw.githubusercontent.com/GaryMilne/Hubitat-Tasmota/main/Triple_Switch_Plug.groovy", singleThreaded: true )  {
        capability "Switch"
        capability "Refresh"
    ...
    section("Configure the Inputs"){
			input name: "destIP", type: "text", title: bold(dodgerBlue("Tasmota Device IP Address")), description: italic("The IP address of the Tasmota device."), defaultValue: green("192.168.0.X"), required:true, displayDuringSetup: true
...
def on() {
    if (settings.switchBehaviour == "1") {
        ...
        command = "POWER1 ON; POWER2 ON; POWER3 ON"
...
def off() {
    if (settings.switchBehaviour == "1") {
        ...
        command = "POWER1 OFF; POWER2 OFF; POWER3 OFF"
        callTasmota("BACKLOG", command)
        }
	}
        ...
        switch(Action.toUpperCase()) { 
            ...
            case ["POWER2"]:
                ...
                if (ActionValue.equalsIgnoreCase(body.POWER2) ){
                    ...
                    if ( ActionValue.toLowerCase() == "on" ) state.lastOnSwitch2 = new Date().format('MM-dd HH:mm:ss')
                    if ( ActionValue.toLowerCase() == "off" ) state.lastOffSwitch2 = new Date().format('MM-dd HH:mm:ss')
                } 
            else {
			...
			case ["POWER3"]:
                ...
                if (ActionValue.equalsIgnoreCase(body.POWER3) ){
                    ...
                    if ( ActionValue.toLowerCase() == "on" ) state.lastOnSwitch3 = new Date().format('MM-dd HH:mm:ss')
                    if ( ActionValue.toLowerCase() == "off" ) state.lastOffSwitch3 = new Date().format('MM-dd HH:mm:ss')
                } 
            else {
...
def tasmotaInjectRule(){
    ...
    rule3 = "ON Power1#State DO backlog0 Var8 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Power2#state DO backlog0 Var9 %value% ; RuleTimer1 1 ENDON "
	rule3 = rule3 + "ON Power3#state DO backlog0 Var10 %value% ; RuleTimer1 1 ENDON "
    rule3 = rule3 + "ON Rules#Timer=1 DO Var15 %Var8%,%Var9%,%Var10% ENDON "
    rule3 = rule3 + "ON Var15#State\$!%Var16% DO backlog ; Var16 %Var15% ; webquery http://" + settings.HubIP + ":39501 POST {'TSync':'True','Switch1':'%Var8%','Switch2':'%Var9%','Switch3':'%Var10%'} ENDON "
