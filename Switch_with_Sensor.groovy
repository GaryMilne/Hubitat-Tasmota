...
*
*  SWITCH w/SENSOR - UNIQUE - CHANGELOG
...
metadata {
		definition (name: "Tasmota Sync - Switch with Sensor", namespace: "garyjmilne", author: "Gary J. Milne", importUrl: "https://raw.githubusercontent.com/GaryMilne/Hubitat-Tasmota/main/Tasmota%20Sync%20Switch%20with%20sensor%20-%20Current%20Version.txt", singleThreaded: true )  {
...
def syncTasmota(body){
    log ("syncTasmota", "Data received: ${body}", 0)    // Body looks something like: {"TSYNC":"TRUE","SWITCH":"0","TEMPERATURE":"67","HUMIDITY":"43","DEWPOINT":"44"}
    ...
    if (elapsed > settings.debounce){
        ...
        //Only changes will get logged so we can report everything. In Tasmota, "power" is the switch state but we use "SWITCH" in the TSYNC JSON.
		...
		body.each {
            try {
                ...
                if (it.key.toUpperCase() != "SWITCH" && it.key.toUpperCase() != "TSYNC" ) {
...
def tasmotaInjectRule(){
    ...
    //First line of rule3 handles changes in power state.
    rule3 = "ON Power1#state DO backlog var8 %value% ; RuleTimer1 1 ENDON "
    ...
    rule3 = rule3 + "ON Rules#Timer=1 DO var15 '%var8%'," + varString1 + " ENDON "  
    
    //In the prior version this looked like: rule3 = rule3 + "ON var15#State\$!%var16% DO backlog ; var16 %var15% ; webquery http://" + settings.HubIP + ":39501/ POST {'TSync':'True','Switch':'%var11%','Temperature':'%var12%','Humidity':'%var13%','DewPoint':'%var14%'} ENDON "
    rule3 = rule3 + "ON var15#State\$!%var16% DO backlog ; var16 %var15% ; webquery http://" + settings.HubIP + ":39501/ POST {'TSync':'True','Switch':'%var8%'," + varString2 + "} ENDON "
